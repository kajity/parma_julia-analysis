...
elseif target == :all
  ...
  _, events_crab = plot_detected_events_crab!(ax, energy, altitude=altitude, label="Crab photon", n_bin=n_bin, area=100., bin_max=bin_max, type=plot_type, magnification=telescope_magnification, exposure_time=time)
  material = :cadmium
