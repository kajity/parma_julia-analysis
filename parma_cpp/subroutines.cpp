...
// *******************************************************
{
  ...
  if (s >= 0) {
    ...
    getFFPfromW = 370.0 - 3.0e-1 * pow(std::abs(s), 1.45);  // FFP in MV
  }
  return getFFPfromW;
}
...
// *******************************************************
{
  ...
  std::string chatmp;
  static std::string pname[npart + 1] = {"neutro", "proton", "alphaa", "elemag", "elemag", "elemag", "muon--", "muon--", "ions  ", "ions  ", "ions  ", "ions  ", "ions  ", "ions  "};
  ...
  if (ifirst == 0) {  // Initialization
    ...
    for (i = 0; i <= 7; i++) {
      ...
      std::getline(ifs, str);  // Kara-yomi
      for (ib = 1; ib <= nBdata; ib++) {
        std::getline(ifs, str);  // read data
    ...
    std::getline(ifs1, str);  // Kara-yomi
    for (i = npart - 5; i <= npart; i++) {
      for (ib = 1; ib <= nBdata; ib++) {
        std::getline(ifs1, str);  // read data
...
// *******************************************************
{
  ...
  if (p[1] == 0) {  // Initialization
    ...
    std::getline(ifs2, str);  // Kara-yomi
    std::getline(ifs2, str);  // Read data
    std::stringstream s20(str);
    s20 >> chatmp >> p[4] >> p[5] >> p[6];
    std::getline(ifs2, str);  // Read data
    std::stringstream s21(str);
    ...
    std::stringstream s30(str);
    s30 >> p[15] >> p[16] >> p[17] >> p[18] >> p[19];
    std::getline(ifs3, str);  // Read data
    std::stringstream s31(str);
...
// *******************************************************
{
  ...
  if (B[2] == 0.0) {  // first time
    ...
    std::getline(ifs1, str);  // Kara-yomi
    std::getline(ifs1, str);  // Kara-yomi
    std::getline(ifs1, str);  // read B(5) data
    std::stringstream s1(str);
    s1 >> chatmp;
    for (ia = 1; ia <= nAdata; ia++) {
      s1 >> A[ia];
...
// *******************************************************
{
  ...
  if (B[4] == 0.0) {  // first time
    ...
    std::getline(ifs1, str);  // Kara-yomi
    std::getline(ifs1, str);  // Kara-yomi B(1)
    std::getline(ifs1, str);  // Kara-yomi B(2)
    std::getline(ifs1, str);  // Kara-yomi B(3)
    std::getline(ifs1, str);  // Kara-yomi B(4)
    std::getline(ifs1, str);  // Kara-yomi B(5)
    for (ib = 1; ib <= 3; ib++) {
      std::getline(ifs1, str);  // read B data
      std::stringstream s1(str);
      s1 >> chatmp;
      for (ia = 1; ia <= nAdata; ia++) {
        s1 >> A[ib][ia];
...
// *******************************************************
{
  ...
  std::string chatmp;
  static std::string pname[npart + 1] = {"neutro", "proton", "alphaa", "elemag", "elemag", "elemag", "ions  ", "ions  ", "ions  ", "ions  ", "ions  ", "ions  "};
...
// *******************************************************
{
  ...
  if (g < 0.0) {
    ...
    fG = fG * (std::abs(gtmp) - 10 * int(std::abs(gtmp) / 10.0)) / airbus;  // consider size of aircraft
  }
...
// *******************************************************
{
  ...
  if (x <= atlow) {  // backward
    funcAng = a[1] + a[2] * pow(std::abs(x), a[3]);
  } else if (x < athig) {  // intermediate
    tmp1 = a[1] + a[2] * pow(std::abs(atlow), a[3]);
...
// *******************************************************
{
  ...
  sum1 = std::max(0.0, getintCpp(std::abs(atlow), one, a[1], a[2], a[3]));                      // -1.0 to atlow
  sum2 = std::max(0.0, (funcAngCpp(atlow, a) + funcAngCpp(athig, a)) * 0.5 * (athig - atlow));  // atlow to athig
  sum3 = std::max(0.0, getintCpp(athig, a[7], a[4], a[5], a[6]));                               // athig to a(7)
  if (a[7] < one) {
    sum4 = std::max(0.0, (funcAngCpp(a[7], a) + funcAngCpp(one, a)) * 0.5 * (one - a[7]));  // a(7) to 1.0
  } else {
    sum4 = 0.0;
  }
...
// *******************************************************
{
  ...
  if (i == 7) {
    getParaAdep = std::max(0.1, std::min(1.0, getParaAdep));
...
// *******************************************************
{
  ...
  static std::string pname[npart + 1] = {"      ", "neutro", "proton", "he---4", "muon--", "elepos", "photon"};
...
// *******************************************************
{
  ...
  elog = std::max(4.062, log10(emid));  // parameters are effective only between 10GeV to 1 TeV
        ...
        for (i = 1; i <= maxfit; i++) {
          if (e == 0.0) {  // energy integrated
            ...
            if (i == 2 && (ParaAdep[1][idx] + ParaAdep[2][idx]) < phimin)
              ParaAdep[2][idx] = -ParaAdep[1][idx] + std::min(ParaAdep[1][idx], phimin);  // avoid negative value at cos(theta)=-1.0
            if (i == 5 && (ParaAdep[4][idx] + ParaAdep[5][idx]) < phimin)
              ParaAdep[5][idx] = -ParaAdep[4][idx] + std::min(ParaAdep[4][idx], phimin);  // avoid negative value at cos(theta)=1.0
          }
        }
      }
    }
...
// ************************************************
{
  ...
  getHP = 0.0;
  if (ic == 4) {
    std::cout << "No FFP data is available" << "\n";
  }
  return getHP;
}
...
// ******************************************************
{
  ...
  if ((cido < -90.01 || cido > 90.01) || iMSIS == 0) {  //! US standard atmosphere 1976 mode
    ...
    if (ia == maxUS + 1) {  // ! out of range
      ...
      std::cout << "Altitude = " << alti << " (km) is too high. It is assumed to be " << altUS[maxUS] << " (km)" << "\n";
      getd = depUS[maxUS];
      return getd;
    }
    ...
    if (ia == maxMSIS + 1) {  // ! out of range
      std::cout << "Warning in function getd" << "\n";
      std::cout << "Altitude = " << alti << " (km) is too high. It is assumed to be " << altMSIS[maxMSIS] << " (km)" << "\n";
