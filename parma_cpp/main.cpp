...
int main()
{
  ...
  dname = DATADIR "/Condition/" + condition + ".inp";
  ifstream conf(dname, ios::in);
  if (!conf) {
    cout << "Error: Cannot open " << dname << "\n";
    return 1;
  }
  getline(conf, str);
  ...
  // open output files
  ofstream sf(DATADIR "/SpecOut/" + condition + ".out", ios::out);
  ...
  ofstream df(DATADIR "/DoseOut/" + condition + "." + dccname, ios::out);
  ...
  ofstream af(DATADIR "/AngOut/" + condition + ".out", ios::out);
  if (isout == 2) {
    for (i = 1; i <= nang + 1; i++) {
      af << setw(14) << i;
    }
  ...
  ifstream dccf(DATADIR "/dcc/" + dccname + ".inp", ios::in);
  if (!dccf) {
    cout << "Error: Cannot open " << dccname << "\n";
    return 1;
  }
  getline(dccf, str);
  getline(dccf, str);
